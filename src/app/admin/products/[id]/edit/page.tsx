...
import { fetchProductById, updateProduct, fetchCategories, uploadProductImages, fetchAllCategories, Category } from "@/lib/adminApi";
...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [form, setForm] = useState({
    ...
    tags: "", tag: "", status: "active" as "active" | "draft" | "archived",
  ...
  useEffect(() => {
    ...
    fetchProductById(id).then(({ product: p }) => {
      setForm({
        name: p.name, description: p.description || "", longDescription: p.longDescription || "",
        category: p.category, categoryId: (p as unknown as {categoryId?: string}).categoryId || "", material: p.material || "", sku: p.sku || "",
        price: String(p.price), originalPrice: p.originalPrice ? String(p.originalPrice) : "",
        stockCount: String(p.stockCount || 0), inStock: p.inStock,
        image: p.image || "", dimensions: p.dimensions || "", weight: p.weight || "",
        rating: String(p.rating || 0), reviewCount: String(p.reviewCount || 0),
        tags: (p.tags || []).join(", "), tag: p.tag || "", status: p.status,
        isFeatured: (p as unknown as {isFeatured?: boolean}).isFeatured || false,
        isTopSeller: (p as unknown as {isTopSeller?: boolean}).isTopSeller || false,
        seo: {
  ...
  if (fetching) {
    return <div className="flex h-64 items-center justify-center"><div className="h-8 w-8 animate-spin rounded-full border-4 border-stone-300 border-t-[#8B4513]" /></div>;
  }

  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/admin/products" className="rounded-lg p-2 text-stone-500 hover:bg-stone-100"><ArrowLeft className="h-5 w-5" /></Link>
        <div>
          <h1 className="text-xl font-bold text-stone-800">Edit Product</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="rounded-xl border border-stone-200 bg-white p-5 shadow-sm">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <div>
              ...
              <input value={form.dimensions} onChange={(e) => set("dimensions", e.target.value)} className="w-full rounded-lg border border-stone-300 px-3 py-2 text-sm focus:border-[#8B4513] focus:outline-none focus:ring-2 focus:ring-[#8B4513]/20" />
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium text-stone-700">Weight</label>
              <input value={form.weight} onChange={(e) => set("weight", e.target.value)} className="w-full rounded-lg border border-stone-300 px-3 py-2 text-sm focus:border-[#8B4513] focus:outline-none focus:ring-2 focus:ring-[#8B4513]/20" />
            </div>
          </div>
        ...
        <div className="rounded-xl border border-stone-200 bg-white p-5 shadow-sm">
          ...
          <div className="flex flex-wrap gap-3">
            {images.map((img, i) => (
              <div key={i} className="group relative h-20 w-20 overflow-hidden rounded-lg border border-stone-200">
                ...
                <button type="button" onClick={() => removeImage(i)} className="absolute -right-1 -top-1 rounded-full bg-red-500 p-0.5 text-white opacity-0 transition-opacity group-hover:opacity-100"><X className="h-3 w-3" /></button>
            ...
            <label className={`flex h-20 w-20 cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-stone-300 text-stone-400 hover:border-[#8B4513] hover:text-[#8B4513] ${uploadingImages ? "opacity-50" : ""}`}>
              <Upload className="h-5 w-5" /><span className="text-[9px]">Upload</span>
          ...
          <div className="mt-3">
            ...
            <input value={form.image} onChange={(e) => set("image", e.target.value)} className="w-full rounded-lg border border-stone-300 px-3 py-2 text-sm focus:border-[#8B4513] focus:outline-none focus:ring-2 focus:ring-[#8B4513]/20" />
          </div>
        </div>
        ...
        <div className="rounded-xl border border-stone-200 bg-white p-5 shadow-sm">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input value={form.tags} onChange={(e) => set("tags", e.target.value)} className="w-full rounded-lg border border-stone-300 px-3 py-2 text-sm focus:border-[#8B4513] focus:outline-none focus:ring-2 focus:ring-[#8B4513]/20" />
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium text-stone-700">Badge Tag</label>
              <input value={form.tag} onChange={(e) => set("tag", e.target.value)} className="w-full rounded-lg border border-stone-300 px-3 py-2 text-sm focus:border-[#8B4513] focus:outline-none focus:ring-2 focus:ring-[#8B4513]/20" />
            </div>
            <div>
        ...
        <div className="flex justify-end gap-3">
          ...
          <button type="submit" disabled={loading} className="flex items-center gap-2 rounded-lg bg-stone-900 px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#8B4513] disabled:opacity-50">
            <Save className="h-4 w-4" /> {loading ? "Saving..." : "Update Product"}
          </button>
        </div>
      </form>
    </div>
